...
export const getImageData = ({
  ...
  if (rng() > 0.5) {
    data.hairs = data.hairs.concat(
      generateHairLines2(data.computedFacePoints, numHairLines[2] * 3 + 10, rng)
    )
  }
  // 头发颜色：随机决定头发颜色，可能是自然色或彩虹渐变色，并设置染色的偏移量。
  if (rng() > 0.5) {
    data.hairs = data.hairs.concat(
      generateHairLines3(data.computedFacePoints, numHairLines[3] * 3 + 10, rng)
    )
  }
  // 鼻子：随机生成左右鼻孔的中心点坐标。
