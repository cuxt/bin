...
export function generateFaceCountourPoints(numPoints = 100, rng: () => number) {
  const faceSizeX0 = randomFromInterval(50, 100, rng);
  const faceSizeY0 = randomFromInterval(70, 100, rng);


  const faceSizeY1 = randomFromInterval(50, 80, rng);
  const faceSizeX1 = randomFromInterval(70, 100, rng);
  const faceK0 = randomFromInterval(0.001, 0.005, rng) * (rng() > 0.5 ? 1 : -1);
  const faceK1 = randomFromInterval(0.001, 0.005, rng) * (rng() > 0.5 ? 1 : -1);
  const face0TranslateX = randomFromInterval(-5, 5, rng);
  const face0TranslateY = randomFromInterval(-15, 15, rng);

  const face1TranslateY = randomFromInterval(-5, 5, rng);
  const face1TranslateX = randomFromInterval(-5, 25, rng);
  const results0 = getEggShapePoints(faceSizeX0, faceSizeY0, faceK0, numPoints, rng);
  const results1 = getEggShapePoints(faceSizeX1, faceSizeY1, faceK1, numPoints, rng);
